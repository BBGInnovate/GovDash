...
#    if self.since_date < 1.month.ago
#      return graph
#    end
    ...
    # tokens = self.api_tokens.select("id, account_id, canvas_url,api_user_email,page_access_token").
    #   where("api_user_email != 'odditech@bbg.gov' AND page_access_token is not null").to_a
    
    tokens = self.api_tokens.select("id, account_id, canvas_url,api_user_email,page_access_token").
       where("page_access_token is not null").to_a

    if tokens.empty?
      return nil
    else
      token = tokens[self.id % tokens.size]
      self.token_count = {token.api_user_email => 1} 
  ...
  def exchange_page_access_token(access_token=nil)
    ...
    begin
      page_token = graph_api(access_token).graph_call("v2.1/#{self.obj_name}?fields=access_token&access_token=#{token}")
      if page_token['access_token']
        self.update_attributes :page_access_token => page_token['access_token']
      else
        self.update_attributes :user_access_token => access_token if access_token
