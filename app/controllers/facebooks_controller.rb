...
class FacebooksController < ApplicationController
  ...
  def edit
    email="api_user_email='#{params[:email]}'"
    session[:email] = email
    @uri = URI.parse Facebook.config[:canvas_url]
    records = ApiToken.includes(:facebook_account).
      where("canvas_url='#{@uri.host}'").
  ...
  def create
    ...
    if session[:account_id]
      app = ApiToken.includes(:facebook_account).
        references(:facebook_account).
        find_by account_id: session[:account_id],
              api_user_email: session[:email],
              canvas_url: uri.host
      if app
        ...
        ApiToken.includes(:facebook_account).
          where("api_tokens.id != #{app.id} AND canvas_url='#{uri.host}'").
          where("api_user_email='#{session[:email]}'").
          references(:facebook_account).to_a.each do |a|
            if a.facebook_account
              a.exchange_page_access_token access_token.access_token
              # a.facebook_account.exchange_page_access_token access_token.access_token
              a.facebook_account.update_attributes :user_access_token=>access_token.access_token,
                :page_access_token=>a.page_access_token
            end
        end
      end
    end
=begin
    if session[:account_id]
      ...
      if @account
        @account.update_attribute :user_access_token, access_token.access_token
        @account.exchange_page_access_token
        
        FacebookAccount.where("id != #{@account.id}").each do |a|
          a.exchange_page_access_token access_token.access_token
        end
      end
    else
      logger.info "FacebooksController#create no session[:account_id]"
    end
=end
